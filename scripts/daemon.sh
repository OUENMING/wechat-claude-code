...
macos_start() {
  ...
  for var in \
    ANTHROPIC_AUTH_TOKEN ANTHROPIC_API_KEY ANTHROPIC_BASE_URL CLAUDE_API_KEY \
...
"
  ...
  # WeChat bridge always needs these three — not inherited from shell
  plist_extra_env="${plist_extra_env}    <key>CLAUDE_CODE_DISABLE_EXPERIMENTAL_BETAS</key>
    <string>1</string>
    <key>CLAUDE_CODE_ATTRIBUTION_HEADER</key>
    <string>0</string>
    <key>CLAUDE_CODE_DISABLE_GIT_INSTRUCTIONS</key>
    <string>1</string>
"
...
"
  ...
  # WeChat bridge always needs these three — not inherited from shell
  extra_env="${extra_env}Environment=CLAUDE_CODE_DISABLE_EXPERIMENTAL_BETAS=1
