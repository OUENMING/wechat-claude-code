...
export function handleModel(ctx: CommandContext, args: string): CommandResult {
  if (!args) {
    return { reply: '用法: /model <模型名称>\n例: /model haiku 或 /model sonnet 或 /model opus', handled: true };
  }
  // Normalize model name: only accept known short names
  ...
  const model = validModels[normalized];
  if (!model) {
    return { reply: `❌ 无效模型名: ${args}\n支持的模型: haiku, sonnet, opus, fable\n例: /model haiku`, handled: true };
  }
  ctx.updateSession({ model });
  return { reply: `✅ 模型已切换为: ${model}`, handled: true };
}

export function handleEffort(ctx: CommandContext, args: string): CommandResult {
  if (!args) {
    const current = ctx.session.effort ?? '默认（不覆盖）';
    return { reply: `当前思考强度: ${current}\n用法: /effort <none|low|medium|high|max>\n例: /effort low`, handled: true };
  }
  const normalized = args.trim().toLowerCase();
  const valid: Record<string, string> = {
    'none': 'none', 'low': 'low', 'medium': 'medium', 'high': 'high', 'max': 'max',
  };
  const effort = valid[normalized];
  if (!effort) {
    return { reply: `❌ 无效强度: ${args}\n支持的: none, low, medium, high, max\n例: /effort low`, handled: true };
  }
  ctx.updateSession({ effort });
  return { reply: `✅ 思考强度已设为: ${effort}`, handled: true };
}
